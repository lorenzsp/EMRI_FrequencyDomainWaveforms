...
def run_emri_pe(
    ...
    [few_gen(*injection_in, **emri_kwargs) for _ in range(1)]
    toc = time.perf_counter()
    fd_time = toc-tic
    print('fd time', fd_time/1)
    ...
    tic = time.perf_counter()
    [td_gen(*injection_in, **emri_kwargs) for _ in range(1)]
    toc = time.perf_counter()
    fd_time = toc-tic
    print('td time', fd_time/1)
    ...
    def loglike_mp(x):
        sig_fd = like_gen(*x, **emri_kwargs)
        ll = [data_stream[0]-sig_fd[0],data_stream[1]-sig_fd[1]]
        return -0.5 * inner_product(ll, ll, **fd_inner_product_kwargs)
